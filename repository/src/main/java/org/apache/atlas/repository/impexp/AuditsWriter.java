...
public class AuditsWriter {
    ...
    private final ExportImportAuditService auditService;

    private final ExportAudits auditForExport = new ExportAudits();
    private final ImportAudits auditForImport = new ImportAudits();
    ...
    static class ReplKeyGuidFinder {
        ...
        static String extractHiveDBQualifiedName(String qualifiedName) {
            return String.format("%s@%s", StringUtils.substringBefore(qualifiedName, "."), StringUtils.substringAfter(qualifiedName, "@"));
        }
    ...
    private class ExportAudits {
        public void add(String userName, AtlasExportResult result, long startTime, long endTime, List<String> entityGuids) throws AtlasBaseException {
            ...
            boolean            replicationOptionState    = request.isReplicationOptionSet();
            boolean            skipUpdateReplicationAttr = request.skipUpdateReplicationAttr();

            saveCurrentServer();

            String targetServerFullName = request.getOptionKeyReplicatedTo();
            String targetServerName     = getServerNameFromFullName(targetServerFullName);

            auditService.add(userName, getCurrentClusterName(), targetServerName, ExportImportAuditEntry.OPERATION_EXPORT, AtlasType.toJson(result), startTime, endTime, !entityGuids.isEmpty());

            if (result.getOperationStatus() == AtlasExportResult.OperationStatus.FAIL) {
                return;
            }

            updateReplicationAttribute((replicationOptionState && !skipUpdateReplicationAttr), targetServerName, targetServerFullName, entityGuids, Constants.ATTR_NAME_REPLICATED_TO, result.getChangeMarker());
        }
    }
    ...
    private class ImportAudits {
        public void add(String userName, AtlasImportResult result, long startTime, long endTime, List<String> entityGuids) throws AtlasBaseException {
            ...
            String sourceServerFullName = request.getOptionKeyReplicatedFrom();
            String sourceServerName     = getServerNameFromFullName(sourceServerFullName);

            auditService.add(userName, sourceServerName, getCurrentClusterName(), ExportImportAuditEntry.OPERATION_IMPORT, AtlasType.toJson(result), startTime, endTime, !entityGuids.isEmpty());
        ...
        public void add(String userName, String sourceCluster, long startTime, long endTime, Set<String> entityGuids) throws AtlasBaseException {
            String sourceServerName = getServerNameFromFullName(sourceCluster);

            auditService.add(userName, sourceServerName, getCurrentClusterName(), ExportImportAuditEntry.OPERATION_IMPORT_DELETE_REPL, AtlasType.toJson(entityGuids), startTime, endTime, !entityGuids.isEmpty());
        }
    }
}
